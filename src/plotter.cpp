...
void CGraph::CairoGraph::draw_series(const Cairo::RefPtr<Cairo::Context>& cr) {
    ...
    if (true == draw_zoom) {
        if (plot.zoom_end_x < plot.zoom_start_x)
            std::swap(plot.zoom_start_x, plot.zoom_end_x);
        if (plot.zoom_end_y < plot.zoom_start_y)
            std::swap(plot.zoom_start_y, plot.zoom_end_y);
