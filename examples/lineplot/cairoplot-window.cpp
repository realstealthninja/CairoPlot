...
CairoplotWindow::CairoplotWindow(const Glib::RefPtr<Gtk::Application>& app) : Glib::ObjectBase("CairoplotWindow"), Gtk::ApplicationWindow(), m_app(app) {
    ...
    selectlinestyle->signal_changed().connect((sigc::track_obj([this]() {
        ...
        case 0:
            if (single_series)
                graph->set_line_style(0, CairoGraphLineStyle::SOLID_LINE);
            else
                for (size_t i = 0; i < 3; ++i)
                    graph->set_line_style(i, CairoGraphLineStyle::SOLID_LINE);
            break;
        case 1:
            if (single_series)
                graph->set_line_style(0, CairoGraphLineStyle::DASHED_LINE);
            else
                for (size_t i = 0; i < 3; ++i)
                    graph->set_line_style(i, CairoGraphLineStyle::DASHED_LINE);
            break;
        case 2:
            if (single_series)
                graph->set_line_style(0, CairoGraphLineStyle::CIRCLE);
            else
                for (size_t i = 0; i < 3; ++i)
                    graph->set_line_style(i, CairoGraphLineStyle::CIRCLE);
            break;
        case 3:
            if (single_series)
                graph->set_line_style(0, CairoGraphLineStyle::DOTS);
            else
                for (size_t i = 0; i < 3; ++i)
                    graph->set_line_style(i, CairoGraphLineStyle::DOTS);
            break;
        default:
            break;
        }
    ...
    selectgraph->signal_changed().connect((sigc::track_obj([this]() {
        if (0 == selectgraph->get_active_row_number())
            single_series = true;
        else
            single_series = false;
        make_plot();
    ...
    scale2.set_tooltip_markup(_("The time (in dimensionless units of <i>t</i><sub>C</sub> |<i>V</i> | ) the perturbation is switched off."));
    ...
    scalelabel2.set_tooltip_markup(_("The time (in dimensionless units of <i>t</i><sub>C</sub> |<i>V</i> | ) the perturbation is switched off."));
...
void CairoplotWindow::about() {
    ...
    // aboutdialog->set_logo(Gdk::Texture::create_for_pixbuf(Gdk::Pixbuf::create_from_resource("/org/gnome/plotter/plotter.png", 128, 128, true)));
...
bool CairoplotWindow::make_plot2() {
    ...
    for (size_t k = 0; k < numplots; ++k) {
        ...
        for (size_t i = 0; i < size; ++i) {
            xseries[k][i] = t;
            (t < t_cutoff) ? yseries[k][i] = 4.0 * V * V * std::pow(std::sin(Omega * t * 0.5), 2) / Omega_sq
                           : yseries[k][i] = 4.0 * V * V * std::pow(std::sin(Omega * t_cutoff * 0.5), 2) / Omega_sq;
            t += 0.05;
        }
    }
    ...
    std::get<0>(textobject[0]) = Glib::ustring("<i>t</i><sub>C</sub> |<i>V</i> | = ") + Glib::ustring::format(std::fixed, std::setprecision(2), scale2.get_value());
...
void CairoplotWindow::make_plot() {
    ...
    if (true == single_series) {
        ...
        // 							1.0);
        Gdk::RGBA col;
        col.set_rgba(linecolour->get_rgba().get_red(),
        ...
        if (false == hidelegend->get_active())
            graph->add_single_legend(legend, CairoGraphLegendPosition::LEGEND_TOP, true);
    } else {
        std::vector<Glib::ustring> legends(numplots);
        ...
        if (false == hidelegend->get_active())
            graph->add_multi_legends(legends);
    }
