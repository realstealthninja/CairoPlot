...
void CGraph::CairoGraph::on_button_press_event(int button, double x, double y) {
    ...
    if (button == 1) {
        if (scaled_x > OFFSET_X && scaled_x < OFFSET_X + GRAPH_WIDTH && scaled_y > OFFSET_Y && scaled_y < OFFSET_Y + GRAPH_HEIGHT) {
            start_x = scaled_x;
            start_y = scaled_y;
...
void CGraph::CairoGraph::on_button_release_event(int button_press, double, double) {
    if (button_press == 1) {
        ...
        if (plot.zoom_factor_x < 0.00001 || plot.zoom_factor_y < 0.00001)
            return;
    }
...
void CGraph::CairoGraph::on_motion_notify_event(double current_width, double current_height) {
    ...
    if (scaled_x > OFFSET_X && scaled_x < OFFSET_X + GRAPH_WIDTH && scaled_y > OFFSET_Y && scaled_y < OFFSET_Y + GRAPH_HEIGHT && true == selection_mode) {
        end_x = scaled_x;
        end_y = scaled_y;
        queue_draw();
    }
